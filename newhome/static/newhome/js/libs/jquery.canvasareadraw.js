(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    $canvas = $('<canvas id="draw-canvas" style="position: absolute; left: 0; top: 0; padding: 0; border: 0 none; z-index: 100;">');
    ctx = $canvas[0].getContext('2d');

    var $image = settings.imageObj;
    ...
    draw = function() {
      ...
      ctx.fillStyle = 'rgb(255,27,85)';
      ctx.strokeStyle = 'rgb(255,27,85)';
      ctx.lineWidth = 2;
      ...
      ctx.closePath();
      ctx.fillStyle = 'rgba(255,27,85,0.3)';
      ctx.fill();
      ctx.stroke();
    ...
    $(input).on('change', function() {
      ...
      settings.imageObj.before($canvas);
