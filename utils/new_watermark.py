...
# взято из django-easy-thumbnails-watermark, пропатчен watermark_processor
...
def watermark_filter(img, mark, position, opacity, scale, tile, greyscale, rotation, **kwargs):
    ...
    if scale[0] != mark.size[0] and scale[1] != mark.size[1]:
        mark = mark.resize(scale, Image.LANCZOS)
...
def watermark_processor(image, WATERMARK=False, **kwargs):
    if WATERMARK:
        path = os.path.join(settings.BASE_DIR, WATERMARK['image'])
        mk = Image.open(path)
